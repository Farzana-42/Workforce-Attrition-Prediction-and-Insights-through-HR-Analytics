...
# --------------------------------------------------------------------------------------
# PATHS (robust across local/Cloud; works if app lives under Scripts/)
# --------------------------------------------------------------------------------------
HERE = Path(__file__).resolve().parent          # .../Scripts
ROOT = HERE.parent                              # repo root
MODEL_CANDIDATES = [
    ROOT / "models" / "random_forest_model.pkl",   # top-level models/
    HERE / "models" / "random_forest_model.pkl",   # Scripts/models/
]
...
def validate_uploaded_data(user_df):
    ...
    # Data types
    if 'Age' in user_df.columns and not pd.api.types.is_numeric_dtype(user_df['Age']):
        validation['errors'].append("'Age' must be numeric")
        validation['is_valid'] = False
        validation['quality_score'] -= 10

    if 'Salary' in user_df.columns and not pd.api.types.is_numeric_dtype(user_df['Salary']):
...
def main():
    ...
    if st.session_state.mode == 'template':
        ...
        with col1:
            ...
            st.download_button("📥 Download Template (CSV)", csv,
                               "employee_attrition_template.csv", "text/csv",
        ...
        with col2:
            ...
            for field, description in field_defs.items():
                doc_text += f"{field}:\n  {description}\n\n"
            st.download_button("📄 Download Field Definitions", doc_text.encode('utf-8'),
                               "field_definitions.txt", "text/plain",
                               use_container_width=True)
        ...
        if st.button("🎯 Generate Attrition Predictions", type="primary", use_container_width=True):
            with st.spinner("Engineering features and generating predictions..."):
                try:
                    ...
                    # Load trained model (robust path resolution)
                    model_path = resolve_model_path()
                    if model_path is None:
                        st.error("""
                        ❌ Model file not found. Please ensure:
                        1. You have trained and saved your model as 'models/random_forest_model.pkl' (or Scripts/models/)
...
def calculate_heuristic_risk(df):
    ...
    # Factor 4: No promotion (with tenure)
    if 'Joining Designation' in df.columns and 'Designation' in df.columns:
...
def display_results(user_df, predictions):
    ...
    available_cols = [c for c in user_df.columns if c not in ['Attrition_Risk_Score', 'Risk_Level']]
    default_cols = []
    for c in ['Emp_ID', 'Age', 'Department', 'City', 'Designation', 'Quarterly Rating']:
    ...
    if display_cols:
        ...
        styled_df = display_df.style.applymap(
            color_risk,
            subset=['Risk_Level'] if 'Risk_Level' in display_df.columns else []
        )
        st.dataframe(styled_df, use_container_width=True, height=400)

    # --- Unified colors and order for charts ---
    ...
    with c1:
        ...
        fig.update_layout(xaxis_title="Risk Score (%)", yaxis_title="Count", title_x=0.3)
        st.plotly_chart(fig, use_container_width=True)

    with c2:
        # Donut (robust: build labels/values explicitly and use go.Pie)
        s_counts = user_df['Risk_Level'].value_counts(dropna=False)

        labels = RISK_ORDER
        values = [int(s_counts.get(k, 0)) for k in RISK_ORDER]
    ...
    with d2:
        ...
        if len(high_risk_df) > 0:
            ...
            st.download_button("⚠️ High Risk Only", csv_high,
    ...
    with d3:
        ...
        if len(at_risk_df) > 0:
            ...
            st.download_button("📊 Action Required", csv_risk,
